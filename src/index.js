...
const table_rows_for = (prfx, record) => {
  ...
  const tableRows = [
    [prefix("Name"), `${record.get("firstName")} ${record.get("lastName")}`],
    [prefix("Gender"), record.get("gender")],
    [
      prefix("ID"),
      _(identifiers)
        .entries()
        .reject(([key, value]) => value.indexOf("-") > -1)
    ...
    [
      ...
      record
        .get("addresses", [])
        .map(address => address.addressFields.address2)
        .join(", ")
    ]
  ];
...
const app = new TurnIntegration(process.env.SECRET)
  ...
  .action(({ chat, messages }) => [
    {
      ...
      callback: ({ message, option, payload: { taskId } }, resp) => {
        debug("Mark a Task menu item clicked!");
        // Notify the frontend to refresh the context automatically
   ...
   */
  .context("OpenSRP Patient Records", "table", ({ chat, messages }) => {
    return new Promise((resolve, reject) => {
      request.get(
        `${process.env.OPENSRP_ENDPOINT}/opensrp/rest/search/path`,
        {
          auth: {
        ...
        function(error, openSrpResponse, openSrpData) {
          if (error) {
            ...
            const tableRows = Object.keys(results).reduce(
              (accumulator, key) => {
